...
fn edit_proposal(key: u64, proposal: CreateProposal) -> Result<(), VoteError> {
    PROPOSAL_MAP.with(|p| {
        let old_proposal_optional = p.borrow.get(&key);
        let mut old_proposal: Proposal;
        match old_proposal_optional {
...
fn vote(key: u64, choice: Choice) -> Result<(), VoteError> {
    PROPOSAL_MAP.with(|p| {
        ...
        if old_proposal.voted.contains(&caller) {
            return Err(VoteError::AlreadyVoted);
        } else if old_proposal.is_active != true {
        ...
        old_proposal.voted.push(caller);
        let res = p.borrow_mut().insert(key, old_proposal);
